...
function go_save(contextPath) 
{
	...
	if (theForm.kind.value == '') {
		...
		console.log(theForm.bestyn.value);
		console.log(theForm.useyn.value);
		...
		theForm.action = contextPath + "/admin/products/adminProductWrite";
		theForm.submit();
	}
}
...
{
	var theForm = document.frm;
	theForm.action = contextPath + "/admin/products/adminProductList";
	theForm.submit();
}
//projectList.jsp
...
function go_detail(contextPath, tpage, pseq) {
	var theForm = document.frm;
	// 상품 상세 보기 페이지에서 다시 상품 리스트로 돌아왔을 경우 현재 페이지로
	// 돌아올 수 있도록 하기 위해서 현재 페이지 번호를 쿼리 스트링으로 넘겨준다.
	theForm.action = contextPath + "/admin/products/adminProductDetail?tpage=" + tpage + "&pseq=" +pseq;
	theForm.submit();
}

function go_wrt(contextPath) {
	var theForm = document.frm;
	theForm.action = contextPath + "/admin/products/adminProductWriteForm";
	theForm.submit();
}
// **************** productDetail.jsp
function go_list(contextPath, tpage) {
	var theForm = document.frm;
	//상품 리스트로 이동하되 현재 페이지를 쿼리 스트링으로 넘긴다.
	theForm.action = contextPath + "/admin/products/adminProductList?tpage=" + tpage;
	theForm.submit();
}
// **************** productDetail.jsp
function go_mod(contextPath, tpage, pseq) {
	var theForm = document.frm;
	//현재 페이지를 쿼리 스트링으로 넘긴다.
	theForm.action = contextPath + "/admin/products/adminProductUpdateForm?tpage=" + tpage + "&pseq=" + pseq;
	theForm.submit();
}

function go_mod_save(contextPath, tpage, pseq) {
	var theForm = document.frm;
	...
	if (theForm.kind.value == '') {
		...
		if (confirm('수정하시겠습니까?')) {
			...
			theForm.action = contextPath + "/admin/products/adminProductUpdate";
			theForm.submit();
		}
	}
}

function go_mod_mov(contextPath, tpage, pseq) {
	var theForm = document.frm;
	theForm.action = contextPath + "/admin/products/adminProductDetail?tpage=" + tpage + "&pseq=" + pseq; 
	theForm.submit();
}
...
function go_deleteItem(url,pseq){
	var theForm = document.frm;
	theForm.action = url + "/admin/products/adminProductDelete?pseq="+pseq;
	theForm.submit();
}
